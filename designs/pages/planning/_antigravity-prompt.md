# Antigravity test runbook — Planning (`planning`)  ·  SPEC_VERSION 2026-06-06.2 (REWORK)
...
1. Open `_status.md` (this folder). It MUST read **`STAGE: TEST`** (Cursor sets this when the build is done) and **`SPEC_VERSION: 2026-06-06.2`**.
2. If it says `STAGE: BUILD` → Cursor hasn't finished building yet — **STOP** and tell the human to let Cursor finish first. If it says `DONE` or an older `SPEC_VERSION` → you're likely in the wrong worktree — **STOP** and tell the human: *"`_status.md` here is `<stage>` / `<version>`, not `TEST` / `2026-06-06.2` — open the worktree with the built page and re-run me. I can't switch worktrees myself."*
...
4. Record every row in `_findings.md` under a new `## <today> — Antigravity — against SPEC_VERSION 2026-06-06.2` heading, one table line per row ID: `| <row id> | PASS/FAIL/SKIP/N/A | note (repro for FAIL) |`.
