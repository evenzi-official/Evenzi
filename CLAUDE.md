...
| Planning Tools (Checklist + Budget) | P2 | Data model LIVE on dev (`planning_01-07`); **FE UI built** (`app/events/[id]/planning`, 833-line `PlanningClient.tsx`, checklist+budget tabs) **but not wired to the backend** — zero `fetch`/Supabase calls in the client, task/expense IDs are plain numbers not DB rows. Corrected 2026-07-30 — docs previously said "FE/app not started," which understated the UI work; real gap is backend wiring, not the UI | 15 |
