...
| Digital Presence (Event Website) | P2 | **Data model DONE (`DATA-MODEL.md` D49–D54). Wave 1 (host editor) + Wave 2 (public API routes) + Wave 3 (guest template routing) all pushed by Dheeraj, 2026-08-02** (`d2ad5a5`, `b5a8389`) — but **Antigravity E2E + a follow-up retest found real bugs still open**: Design page template save (contradictory retest evidence, 200 vs 500 across two runs — treat as unresolved), Venue & Travel travel point save still drops `travel_time_text`, and Q&A/travel-points/stays/guest-lookup were never actually confirmed through a real browser (retest fell back to direct API calls). Story editor + Schedule editor are the only two stages independently confirmed fixed. Full breakdown: `qa/event-website-retest-validation.md`. Registry/Video sections are explicitly not backend-wired yet (by design, "saved in browser only"). One decision still open: `x-forwarded-for` gateway-trust verification (spec §6b.3). | Code pushed, 4 bugs open pending Dheeraj self-verification |
