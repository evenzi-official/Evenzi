...
| Planning Tools (Checklist + Budget) | P2 | **DONE** — backend-wiring pass complete 2026-07-30. 7 new API routes (`app/api/events/[id]/planning/*`: task CRUD, bulk actions, budget, expense CRUD, expense-types), `page.tsx` real server-side fetch, `PlanningClient.tsx` fully wired (real task/expense CRUD, bulk complete/delete/set-date/assign, custom expense-type creator, receipt-upload UI stub). 9-task subagent-driven build, every task reviewed (2 needed a fix+re-review cycle for real bugs: task-route not-found/error-handling, missing optimistic-update rollback — both fixed). Typecheck + lint clean. **Live 6-breakpoint browser verification still pending** — handed to Antigravity, see `docs/superpowers/plans/2026-07-30-planning-tools-fe-wiring.md` Task 9. | 15 |
