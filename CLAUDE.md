...
| Digital Invitations (card designer) | P3 | ✅ **persists** (2026-08-24, branch `feature/invitations-persistence`) — full fidelity: template, 7 text slots, per-line sizes (`inv_07` `slot_sizes` col), + R2 images (public bucket). Debounced autosave (GET/PATCH `/api/events/[id]/invitation-card` + upload-url), resume-editor-if-personalized. | Scope is the card designer only; WhatsApp send lives in Guest Mgmt. **Live-verified:** text+size+template-swap persist across reload. **Not yet live-clicked:** photo-BG + upload-own-card image persistence (code complete, R2 path sound). Sub-event cards + server render still deferred |
