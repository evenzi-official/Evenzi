...
| Digital Presence (Event Website) | P2 | **Data model Wave 1 DONE (LIVE 2026-07-31)** — 13 new tables (`website_01`–`website_11`), council-reviewed, see `DATA-MODEL.md` D49. Host-editor React pages (`app/events/[id]/website/*`) still static mocks — Dheeraj has a wiring brief at `docs/sprint/sprint-1/handoff-website-wave1.md`. **Wave 2** (public `/e/[slug]` site, guest phone+name lookup, `anon` RLS) is spec-only, council verdict 🔴 RE-PLAN — findings in spec §12, revision is next session's job. | Partial |
