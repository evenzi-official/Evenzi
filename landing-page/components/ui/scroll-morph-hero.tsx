...
// Phase constants — container h-[700vh], sticky scroll = 600vh
const PHASE_FEATURES_START = 0.35;
const TOTAL_SLIDE_ITEMS = 8; // heading + 6 features + CTA
const ITEM_RANGE = (1.0 - PHASE_FEATURES_START) / TOTAL_SLIDE_ITEMS; // ≈0.093 each
...
export default function IntroAnimation() {
  ...
  // Phase 1: center text in/out
  const textOpacity = useTransform(scrollYProgress, [0.00, 0.07], [0, 1]);
  const textY = useTransform(scrollYProgress, [0.00, 0.07], [30, 0]);
  const textOpacityOut = useTransform(scrollYProgress, [0.20, 0.30], [1, 0]);
  const textScaleOut = useTransform(scrollYProgress, [0.20, 0.30], [1, 0.85]);

  // Phase 2: circle exits
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[700vh]">
      <div className="sticky top-0 h-screen w-full flex items-center justify-center bg-[#FAFAFA] overflow-hidden">
        ...
        {/* Phase 1: "One workspace. Every celebration." */}
        <motion.div
          style={{ opacity: textOpacityOut, scale: textScaleOut }}
          className="absolute z-10 flex flex-col items-center text-center pointer-events-none px-8"
        >
          <motion.h2
            style={{ opacity: textOpacity, y: textY }}
            className="font-serif font-light text-[clamp(22px,3.5vw,48px)] text-[#080808] leading-[1.3] tracking-tight"
          >
        ...
        {/* Phase 2: circle of images, exits outward */}
        <motion.div
