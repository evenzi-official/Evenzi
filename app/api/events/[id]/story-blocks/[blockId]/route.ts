...
const patchSchema = z.object({
  block_type:    z.enum(['heading', 'photo', 'text']).optional(),
...
export async function PATCH(
  ...
  try {
    ...
    if (!uuidSchema.safeParse(id).success || !uuidSchema.safeParse(blockId).success) {
    ...
    if (error) { console.error('PATCH story-blocks/[blockId] failed:', error); return NextResponse.json({ error: 'Failed to update' }, { status: 500 }) }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) { console.error('DELETE story-blocks/[blockId] failed:', error); return NextResponse.json({ error: 'Failed to delete' }, { status: 500 }) }
