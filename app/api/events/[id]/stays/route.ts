...
export async function GET(
  ...
  try {
    ...
    if (error) { console.error('GET stays failed:', error); return NextResponse.json({ error: 'Failed to fetch' }, { status: 500 }) }
...
export async function POST(
  ...
  try {
    ...
    if (error) { console.error('POST stays failed:', error); return NextResponse.json({ error: 'Failed to create' }, { status: 500 }) }
