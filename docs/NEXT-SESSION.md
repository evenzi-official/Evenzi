...
- **NOT yet UI/UX-agent reviewed:** the Step-3 modals, custom calendar, and time picker shipped without a UI/UX-agent pass (only auth was reviewed). Run the agent over these next session before they harden into the React port.
