...
| **G13** | data-model | **Public site route key** — `app/e/[slug]/` needs something to key off that isn't a raw event UUID. No column existed. | Entire public site | `events.slug text unique`. Live — generation strategy (app-layer) still to be built. | **DONE (schema) — `website_06`** |
