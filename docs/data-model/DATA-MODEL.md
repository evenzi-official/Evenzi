...
| D56 | **Collaborator-aware RLS foundation live (`collab_access_01`–`05`), 2026-08-06/07.** Replaces the D26-deferred "later collaborator pass" for the first 5 tables: `events`, `event_general_settings`, `event_guest_settings`, `event_website_settings`, `event_collaborators`. Two `SECURITY DEFINER` predicates — `can_read_event(event_id, capability)` / `can_write_event(event_id, capability)` — encode the role matrix (owner full; co-host everything except `billing`/`delete`; planner `guests`+`planning`; photographer `media`; viewer read-all/write-none). `event_collaborators.role` CHECK constrained to `('co-host','planner','photographer','viewer')` + partial index on `(user_id, event_id) where status='active'`. `events`: owner ALL + collab SELECT + collab UPDATE (`'general'`) + `trg_guard_events_collab_update` blocking non-owner `user_id`/`deleted_at` changes. Settings sidecars: owner ALL + collab SELECT/write per capability. Collaborators: owner ALL + `collab_self_select` + **SEC-1 split** INSERT/UPDATE/DELETE (no unrestricted `FOR ALL`). App layer: `lib/auth/eventAccess.ts` `getEventAccess`/`requireEventWrite` wired to Settings-domain + core event PUT/DELETE. **Founder lock (2026-08-06):** Delete event + Plan & Billing stay owner-only forever; co-host otherwise has full ops (wedding bride/groom parity). Remaining Planning/Guests/Media/Website-content converted in D57. | D26 said convert all event-children together once collaborator access exists; council (2026-08-06) caught that shipping app-layer capabilities without RLS conversion would silently no-op for every real collaborator. Split predicates (not a single `can_access_event`) close a draft bug where `capability is null` granted blanket read. HARD GATE: policy conversions + Settings route checks landed same sitting. |
| D55 | **`event_general_settings.show_on_dashboard` and `.discoverable` are now unused by design.** `show_on_dashboard` was write-only (no reader anywhere, and even wired it had no path back to a hidden event — no "Hidden events" view exists). `discoverable` was a fully wired round-trip field with no UI control that ever set it. Both removed from `general-settings/route.ts`'s accepted schema and the settings form on 2026-08-06 — columns left in place (nullable-safe, `boolean not null default false/true`), no migration, revisit if a real "hide from dashboard" feature gets designed later. | Dropping columns for genuinely-dead-but-harmless fields isn't worth a migration; removing the API/UI surface is enough. Per Event Settings Cleanup spec §6. || D54 | **`config.website_templates` seeded with `cinematic-scroll` (`website_23`), live 2026-08-02.** D49 deliberately left this catalog empty "until the template lineup locks" — but `event_website_design.template_id` FKs it, so once Dheeraj built the first real guest template (Cinematic Scroll) and tried to save a design, every insert/update failed on the FK violation, and `event_website_design` had 0 rows as a result. Seeded the one live template (`slug='cinematic-scroll'`, `id` left to `gen_random_uuid()` default, not a hardcoded literal — matches this codebase's slug-resolution convention, same as `config.invitation_templates` 'eternal'). **Downstream effect, not a separate bug:** Dheeraj also reported `get_public_website_payload` returning `design: null` — the RPC already builds `design.template_id` correctly (D49/D50), it simply had nothing to return with 0 rows in `event_website_design`. No RPC change needed; fixed automatically once saves succeed. `get_advisors` clean, no new findings; no TS type change (data-only insert, no DDL). | Founder call (Abhijith, 2026-08-02) — insert the real catalog row + resolve by slug in app code, not a hardcoded UUID or dropping the FK to `text`, per D49's already-established "all catalogs are uuid PK + unique slug, no precedent for deviation" ruling. |
| D53 | **Story page tier flipped `private` → `public` (`website_22`), live 2026-08-02.** Closes the last open item in spec §1 — Story and Q&A's tier was never explicitly assigned when `config.website_pages` was seeded in Wave 1, so `story` defaulted to `private` alongside every other content page. Founder confirmed the proposed split (Story=public, Q&A=private) same session as D52; `qa` was already `private` and needed no change, `story` flipped via a single `update`. | Founder call (Abhijith, 2026-08-02) — a couple's "Our Story" is usually shared broadly already (public-tier exposure isn't new risk), while Q&A skews toward logistics/PII-adjacent content (dress code, gifts, parking) that stays private with Wedding Party. |
| D52 | **`events.slug` generator + backfill (`website_21`), live 2026-08-02.** Resolves G13's deferred "generation strategy" — Wave 2's public routes are keyed by `slug`, not `event_id`, but the column had shipped nullable with zero population logic, blocking Dheeraj from testing any `/api/e/[slug]/*` route end-to-end. `public.generate_event_slug(name, primary_date)`: slugify(`name`) + `-` + `primary_date` as `YYYYMMDD` (e.g. `sarah-james-20260815`); on unique-constraint collision, retries with a random 4-char suffix, falling back to 8 chars after 5 attempts as a hard exit from the loop. `SECURITY DEFINER`, `revoke`d from `public`/`anon`/`authenticated` (D50/D51's lesson applied pre-emptively — explicit 3-role revoke, not a bare `from public`), `grant`ed to `service_role` only; never called directly by client code, only from `create_event_with_details` (extended as a new insert-time default) and the one-time backfill. Backfill ran as a `do $$ ... $$` per-row loop (not a single set-based `UPDATE`) so each row's collision check sees the prior row's already-assigned slug within the same transaction — a set-based update would let two same-transaction rows race on the same `not exists` snapshot and collide. Verified against all 19 live events: 19/19 got a slug, all distinct, including a genuine collision (two "test A & test B's Wedding" rows on the same date correctly resolved to `...20260609` and `...20260609-3fa9`). `get_advisors` clean — no new findings, all pre-existing (Wave 2b's intentional `anon` RPC surface). | Founder call (Abhijith, 2026-08-02), triggered by Dheeraj hitting the blocker live while building Wave 2's app-layer routes. Date-in-slug (not just random suffix) was the founder's explicit ask — readable/shareable URLs over opaque ones, with randomness only as a fallback, not the default. |
| D49 | **Event Website (Digital Presence) Wave 1 (`website_01`–`website_10`).** 5 `config.*` catalogs (`website_fonts`/`website_palettes`/`website_templates`/`website_pages`/`website_section_types`) — `anon`+`authenticated` SELECT like every other catalog; `website_pages`/`website_section_types` seeded (10-page spine, 11 section types), `website_fonts`/`website_palettes`/`website_templates` left empty until the template lineup locks. `event_website_design` (1:1 sidecar, `template_id` **nullable** since the catalog starts empty). `event_website_pages`/`event_website_sections` (denormalized `event_id` + guard trigger, matching the 5 existing precedents). **Story/Wedding Party/Q&A got dedicated typed tables** (`event_story_blocks`/`event_wedding_party_members`/`event_qa_items`), not the generic jsonb sections table — founder call, matches the `event_travel_points`/`event_stays` pattern instead. `event_travel_points`/`event_stays` (G2/G3) + `map_link` text columns on `events`/`event_sub_events` (G4 MVP fallback — no lat/lng yet). `events.slug` (G13). `event_guest_sub_events` gained per-sub-event RSVP columns (`response_status`/`plus_one_count`/`dietary_notes`/`responded_at` + a `unique(guest_id, sub_event_id)`) — confirmed a real V0 requirement, not blanket-only. `create_event_with_details` extended (step 9) to seed the page spine per new event. Owner-only RLS throughout — **no `anon` grant on any live table**, only the 5 reference catalogs. Full spec + council review trail: `docs/superpowers/specs/2026-07-30-event-website-data-model-spec.md`. | Council-reviewed 2026-07-30 (Tech Lead/Data Modeller/Security Expert/Backend Engineer, Critique+Debate+Arbiter) after an initial single-pass draft drew 5 critical findings — notably the original draft proposed `anon` read on `event_media`/`event_albums`, directly contradicting D35; removed entirely, Gallery ships in a later slice once Media's signed-URL route exists. Splitting Wave 1/Wave 2 keeps the codebase's first-ever `anon` RLS surface as its own reviewed slice rather than bundling it with deferrable host-editor work. |
| D50 | **Event Website Wave 2a (`website_12`–`website_16`), live 2026-07-31.** Public-payload half of Digital Presence, shipped separately from Wave 2b (guest lookup/session/RSVP — spec'd, council-approved, not yet migrated) per the Wave 2 council's Tech Lead suggestion: `get_public_website_payload(p_slug)` takes no session-token parameter and has zero dependency on `guest_tokens`, so it structurally cannot touch guest identity. `is_website_gate_open()` (site-reachability boolean, never exposes `website_password_hash`), a shared internal `_website_page_content(event_id, tier)` helper (explicit `jsonb_build_object` allow-lists per page type — not `to_jsonb(row)`, which would've leaked `created_by`/`updated_by` host UUIDs), and `ALTER POLICY` tightening Wave 1's 5 catalog policies from unconditional `true` to `enabled = true` (the original Wave 2 draft's `CREATE POLICY` attempt was a no-op stacked on top of the existing unconditional one). Went through 4 full council rounds before migrating (2 on the original design, 2 confirm-the-fixes passes on the revision) — see the spec's §12–§16 for the full trail, including 3 critical bugs a first confirm-the-fixes pass caught in the revision itself (spoofable rate-limit IP-hash, a TOCTOU race in the rate limiter, a Postgres grant gap) and one more the second pass caught (the rate-limit fix's blocking lock had no timeout). **A 5th issue survived all 4 rounds and was only caught by applying the migration:** `_website_page_content` was meant to be internal-only, but `revoke all on function ... from public` didn't actually block `anon`/`authenticated` — Supabase grants `EXECUTE` on new `public`-schema functions to those roles directly via `ALTER DEFAULT PRIVILEGES`, not through the `PUBLIC` pseudo-role, so revoking from `public` alone is a no-op for them. `get_advisors` (security) caught it immediately post-migration; fixed same session (`website_16`, `revoke ... from public, anon, authenticated`). Same category as `website_10`'s guard-trigger fix in D49 — now the second occurrence, not yet generalized into a written rule anywhere in this codebase. | The 4-round review process worked as designed for logic/design-level bugs (rate limiting, grants matching this codebase's `{anon,authenticated}` convention, cross-event checks) — no amount of council review catches a Supabase runtime-privilege-model gap by reading SQL text, since `revoke ... from public` reads as correct and only differs from `revoke ... from public, anon, authenticated` in behavior invisible until applied. This is why `get_advisors` runs immediately after every migration in this project, not just at wave boundaries — it's the only check that observes actual granted privileges rather than the SQL that was intended to set them. Full trail (including the corrected spec text for Wave 2b's `create_guest_token`, which has the same pattern and was fixed pre-emptively): `docs/superpowers/specs/2026-07-30-event-website-data-model-spec.md` §16. |
| D51 | **Event Website Wave 2b (`website_17`–`website_20`), live 2026-07-31.** The codebase's first `anon`-identity surface, completing Digital Presence's DB layer. `guest_tokens` (no FK to `event_guests` — arbiter-ruled, second sanctioned instance of the no-cross-module-FK pattern alongside D23; no guard trigger either, since RLS denies every client-facing role and the only insert path, `create_guest_token()`, validates the guest/event pair inline). `guest_lookup_attempts` — a rate-limit ledger, `bigint` PK (deliberate exception to this codebase's uuid-PK convention: append-only, nothing FK-references it, not client-addressable), zero RLS policies for every role including `authenticated` (stronger than owner-scoped — a host can't read even their own event's attempt log, since it's phone/name fragments belonging to whoever attempted the lookup). `resolve_guest_by_lookup` (phone+name plain match, V0 — no OTP, a founder-accepted tradeoff) enforces rate limiting *inside* the function via `pg_try_advisory_xact_lock` scoped per-event (non-blocking — a caller that can't acquire the lock fails straight into the existing rate-limit error instead of queuing) plus a `guest_lookup_attempts` count check, both per-IP and per-event ceilings. `resolve_guest_session`/`get_guest_website_payload`/`submit_rsvp` all require a valid session token, raise on invalid/expired. All 5 guest-facing RPCs grant `EXECUTE` to `anon`+`authenticated` (matching this codebase's zero-exception convention for guest-facing surfaces — a logged-in host previewing their own site must not get "permission denied"). Went through 4 full council rounds (Tech Lead/Data Modeller/Security Expert/Backend Engineer) before either Wave 2a or 2b migrated — 2 on the original design, 2 confirm-the-fixes passes on the revision — catching 3 critical bugs in the revision itself (spoofable rate-limit IP-hash, a TOCTOU race in the rate limiter, a Postgres anon/authenticated grant gap) plus one more in the fix for those (the rate-limit lock initially had no timeout). **A second live-only-discoverable gap, this time caught before migrating:** `resolve_guest_by_lookup`'s `digest()` call is inside a `search_path = public` function, but `pgcrypto` lives in the `extensions` schema — a bare `digest(...)` call doesn't resolve, confirmed by direct pre-flight execution (`ERROR: function digest(unknown, unknown) does not exist`) before any migration ran. Fixed by schema-qualifying the call site (`extensions.digest(...)`) instead of adding a third `public`-schema wrapper function alongside the pre-existing `public.gen_random_bytes` one (fewer wrapper functions means fewer future instances of D50's class of gap). | Full spec + 4-round council trail + both live-only findings: `docs/superpowers/specs/2026-07-30-event-website-data-model-spec.md` §12–§17. D50's `revoke`-doesn't-revoke gap and this session's `digest()`-doesn't-resolve gap are the same underlying lesson from two different angles — this codebase's `SECURITY DEFINER` + `SET search_path = public` pattern has runtime behavior that doesn't show up from reading the SQL, only from running it. Not yet written into `ai/system/agent_rules.md` as a standing rule — flagged in the spec, not yet acted on. |
...
| D45 | **`_seed_event_settings()` SECURITY DEFINER helper.** D36 threshold reached: adding 3 more `INSERT … ON CONFLICT DO NOTHING` blocks into `create_event_with_details` is the 4th+ catalog-copy seed group. Extracted to `public._seed_event_settings(p_event_id uuid, p_user_id uuid)` (DEFINER, `search_path = public, pg_temp`). REVOKE EXECUTE from `public, anon, authenticated`; GRANT to `service_role` only — the helper is internal to `create_event_with_details` and must not be callable as a public RPC. Called as step 8 of `create_event_with_details`. | `ON CONFLICT DO NOTHING` makes the helper idempotent (safe retry). REVOKE from all client roles prevents a direct RPC call that would bypass the main transaction and RLS. The D36 extraction rule was recorded precisely because this pattern would recur. |
| D44 | **`event_guest_settings`.** `allow_plus_ones` toggle and `max_plus_ones_per_invite` cap are **decoupled at the DB level** — the DB never zeros the cap when the toggle is off (that would silently destroy the host's configured value on toggle-off → toggle-on). The view computes `effective_max_plus_ones` for the RSVP flow (D48). `max_plus_ones_per_invite` is `smallint` (0–10; DB-enforced range check). `default_guest_message = null` means "no message"; service layer **must coerce `""` → `null`** before writes — empty string stored shows a blank message box to guests. | Toggle-cap decoupling is a common UX trap: coupling them destroys user intent on every toggle cycle. Smallint saves 2 bytes vs int for a 0–10 field. Empty-string coercion is a service-layer convention recorded here so it can't be forgotten at FE integration time. |
| D43 | **`event_website_settings`.** `website_password_hash` stores a **bcrypt hash only** — raw password is never persisted; the column name makes this self-documenting. `ck_website_password_required` CHECK ensures the DB is never left in `password_enabled = true, hash = null` state. `ck_announcement_text_required` CHECK ensures `banner_enabled = true` always has non-empty text. Partial indexes on `website_password_enabled` and `site_offline` pre-created for future queries. `website_expires_at` computed in the view (D47) from `primary_date + 60 days` — not stored. | Storing the hash is required for guest unlock via server-side `bcrypt.compare()` (rate-limited, 5 attempts/15 min/IP). CHECK constraints eliminate an entire class of inconsistent-state bugs before they reach the app layer. Partial indexes cost nothing while the table is small. |
| D42 | **`event_general_settings`.** `tagline` lives here (display preference), not on `events` (structural identity). Blank-after-trim rejected by CHECK (null allowed, but `' '` is not). `discoverable` stub default `false` (explicit opt-in when the feature ships). Partial index on `discoverable = true` pre-created so the discovery feature ships without a schema migration. `show_on_dashboard` default `true` (opt-out model — new events are immediately visible). | Tagline is a display preference that may differ per website theme — decoupling it from the structural `events.name` follows D5's derivable-data principle. Pre-creating the partial index is the D36 equivalent: do it when the table is empty, not when the feature launches under load. |
| D41 | **`events.plan_id` FK + `config.free_plan_id()` + `enforce_plan_event_limit` trigger.** Column added nullable → all existing rows backfilled to `free` plan → NOT NULL constraint applied. `config.free_plan_id()` is a STABLE function (PostgreSQL disallows subqueries as column DEFAULTs). `ON DELETE RESTRICT` on the FK prevents deleting a plan tier while events reference it. `enforce_plan_event_limit` is BEFORE INSERT, SECURITY DEFINER — runs as `postgres` so it can read `config.plans`. While `max_events_per_user = NULL`, trigger is a no-op; populating the column activates enforcement with no schema migration. | Nullable-then-backfill-then-NOT-NULL is the safe add-not-null pattern (avoids table lock on large tables). A STABLE function wrapper is the standard workaround for PostgreSQL's column-default subquery restriction. NULL limits = "TBC" keeps the trigger scaffolded without enforcing — one UPDATE to `config.plans` activates it live. |
| D40 | **`config.plans` catalog + `config.plans_public` view.** Three tiers: free (₹0), premium (₹4,900/yr), elite (₹9,900/yr). Limit columns are `int null` (null = TBC / unlimited). Feature flags (`custom_domain`, `priority_support`, `ai_features`) set per prototype; `features jsonb` catch-all for future capability flags. Raw `config.plans` accessible to `authenticated` only (RLS policy); `anon` gets only the `config.plans_public` view (strips internal `features` jsonb, `is_active = true` rows only). | Limits nullable avoids committing to unresearched numbers; the enforcement trigger (D41) skips NULL limits. Two-layer access (raw + public view) is an explicit security boundary: the internal features jsonb is not a customer-facing API. Three tiers match the Event Settings → Plan & Billing prototype. |
...
```
**Notes:** PK = FK `user_id` CASCADE. 2FA is **not** here — it's Supabase Auth MFA. `push_notifications` is the Phase B delivery gate (browser push); Phase A in-app bell always reads `notifications` regardless of this flag.

### `public.notifications`  `[NOW]`
**Purpose:** per-user in-app notification inbox (1 row per recipient). Written only via hardened RPCs — never client INSERT.
```sql
...
-- RLS: select/update own rows only; no insert/delete policies for clients
-- trigger: trg_notifications_updated_at → set_updated_at()
```
**Notes:** Recipients = event owner ∪ active `event_collaborators` minus actor. `link_path` must match `^/events/<uuid>(/…)?$` (enforced in RPC). App APIs: `GET /api/notifications`, `PATCH /api/notifications/[id]`, `POST /api/notifications/mark-all-read`. **Rationale (D55).**
...
Host-side editor for the public guest-facing event website. **Wave 1** — owner-only RLS throughout, no `anon` surface. **Wave 2a/2b (public `/e/[slug]` site, guest self-lookup by phone+name, RSVP submission) are also live** — see D50/D51. `events.slug` generation (deferred at Wave 1 time) shipped D52. **DB layer is complete; `app/api/e/[slug]/*` app-layer routes are the only remaining Digital Presence work.** Full design rationale: [`docs/superpowers/specs/2026-07-30-event-website-data-model-spec.md`](../superpowers/specs/2026-07-30-event-website-data-model-spec.md).
...
### Event Website Wave 2a (`website_12`–`website_16`) — public payload, no guest identity

Went through 4 full council rounds before migrating (2 on the original Wave 2 design, 2 confirm-the-fixes passes on the revision) — full trail in `docs/superpowers/specs/2026-07-30-event-website-data-model-spec.md` §12–§16. Split from Wave 2b (guest lookup/session/RSVP, not yet migrated) so the public-payload half — no `guest_tokens` dependency, no identity risk — could ship on its own lighter review.

#### `is_website_gate_open()` (`website_12`)
...
```
Boolean only — never returns the row, never `website_password_hash`. `website_password_enabled` is a second pre-launch visibility toggle (same category as `site_offline`), not an actual password mechanism — no verification logic exists anywhere in Wave 2.

#### Catalog policy fix (`website_13`)

Wave 1 already granted `anon`+`authenticated` SELECT unconditionally on the 5 `config.website_*` catalogs. The original Wave 2 draft's `CREATE POLICY ... USING (enabled = true)` on top was a no-op (RLS policies for the same command OR together). Fixed by tightening the existing policy in place instead:
```sql
...
#### `_website_page_content()` internal helper (`website_14`)

Shared per-page-slug content join used by both the public and (future, Wave 2b) guest payload RPCs — explicit `jsonb_build_object` allow-lists per page type (story/wedding-party/qa/schedule/venue-travel/registry+video), not `to_jsonb(row)`, which would leak `created_by`/`updated_by` host UUIDs and auto-expose any future column with no review gate. Full SQL: spec §6a.3. Meant to be internal-only — see `website_16` below.

#### `get_public_website_payload(p_slug)` (`website_15`)
...
```
Takes no session-token parameter — structurally cannot touch guest identity, which is what makes it safe to ship ahead of Wave 2b. `design` and page content are explicit allow-lists, same reasoning as `website_14`.

#### `website_16` — internal-helper anon-exec fix

`_website_page_content` was meant to be internal-only (called exclusively by `get_public_website_payload`, and by the Wave 2b guest-payload RPC once it ships), but `revoke all on function ... from public` didn't remove `anon`/`authenticated` EXECUTE — Supabase grants EXECUTE on new `public`-schema functions to those roles directly via `ALTER DEFAULT PRIVILEGES`, not through the `PUBLIC` pseudo-role, so `revoke ... from public` alone is a no-op for them. Same category as `website_10`'s guard-trigger fix, now the second occurrence. Caught by `get_advisors` (security) immediately post-`website_14`, confirmed via `information_schema.routine_privileges`, fixed same session: `revoke execute on function public._website_page_content(uuid, text) from public, anon, authenticated;`. Verified live — only `postgres`/`service_role` retain EXECUTE. **Not yet a written rule anywhere in this codebase** — every future internal-only `SECURITY DEFINER` function should use `revoke ... from public, anon, authenticated` from the start, and `get_advisors` should run immediately after any migration that adds a function, not just at wave boundaries.

### Event Website Wave 2b (`website_17`–`website_20`) — guest lookup, session, RSVP

The codebase's first `anon`-identity surface. Went through 4 full council rounds before either Wave 2a or 2b migrated — full trail: spec §12–§17.

#### `guest_tokens` + `create_guest_token` (`website_17`, `website_19`)

```sql
create table public.guest_tokens (
  id uuid primary key default gen_random_uuid(),
  event_id uuid not null references public.events(id) on delete cascade,
  guest_id uuid not null,   -- no FK — see D51
...
-- Zero policies for every role — only SECURITY DEFINER functions touch this table.
...
```
No FK to `event_guests` — D51/D23's no-cross-module-FK pattern; `guest_id` validated inline by this function instead. No guard trigger — no client-writable path exists for one to guard (RLS denies every role). Revoke explicitly includes `anon, authenticated`, not just `public` — the `website_16` lesson applied from the start this time.

#### `guest_lookup_attempts` + `resolve_guest_by_lookup` (`website_18`, `website_19`)

```sql
create table public.guest_lookup_attempts (
  id bigint generated always as identity primary key,   -- deliberate exception to the uuid-PK convention — see D51
  event_id uuid not null references public.events(id) on delete cascade,
  ip_hash text not null,   -- sha256 of the first x-forwarded-for hop, never the raw IP
...
-- Zero policies for every role including authenticated — a host can't read even their own event's log.
...
language plpgsql as $$
declare
  v_event_id uuid; v_guest_id uuid; v_token text; v_ip_hash text; v_ip_attempts int; v_event_attempts int;
begin
  select id into v_event_id from public.events where slug = p_slug;
  if v_event_id is null or not public.is_website_gate_open(v_event_id) then
  ...
  )), 'sha256'), 'hex');   -- schema-qualified extensions.digest — see D51
  ...
  select id into v_guest_id from public.event_guests
    where event_id = v_event_id
      and regexp_replace(phone, '\D', '', 'g') = regexp_replace(p_phone, '\D', '', 'g')
      and lower(trim(name)) = lower(trim(p_name))
    order by id limit 1;
  if v_guest_id is null then raise exception 'lookup failed'; end if;

  v_token := encode(gen_random_bytes(18), 'base64');   -- public.gen_random_bytes wrapper, pre-existing
...
```
Rate limiting enforced inside the function (not app middleware) — a caller hitting PostgREST directly is limited the same as one going through the app. `pg_try_advisory_xact_lock` is non-blocking, scoped per-event: lock contention fails straight into the existing rate-limit error instead of queuing.

#### `resolve_guest_session` / `get_guest_website_payload` / `submit_rsvp` (`website_20`)

Full SQL: spec §6b.4. `resolve_guest_session` returns the guest's own identity + tagged sub-events + RSVP state. `get_guest_website_payload` returns private-tier page content via `_website_page_content`, gated on a valid session token, never accepts `event_id` directly from the caller. `submit_rsvp` is a plain `UPDATE` (not an upsert — the row is pre-created by Guest Management when the guest is tagged), with an explicit cross-event check (`event_id = v_event_id`) rather than relying on an uncited guard trigger elsewhere. All three raise `'invalid session'` on an invalid/expired token — a deliberate convention distinct from `get_public_website_payload`'s never-error behavior, since these three take a token as their entire reason for being called.
...
| `public._notify_event_recipients(...)` | **[NOW]** (`notifications_01`) | Internal DEFINER fan-out insert into `notifications` for owner + active collabs excluding actor. Revoked from `public`/`anon`/`authenticated`. Validates type CHECK, `link_path` allowlist, title/body length. |
| `public.notify_recipients(...)` | **[NOW]** (`notifications_01`) | Authenticated wrapper: `auth.uid()` must equal `p_actor_id` and caller must own or actively collaborate on the event; then calls `_notify_event_recipients`. `GRANT EXECUTE` to `authenticated` only. |
| `public.submit_rsvp(...)` | **[NOW]** (amended `notifications_02`) | Guest RSVP update; after success, calls `_notify_event_recipients(..., p_actor_id := null)` inside an exception block so notify never fails the RSVP. |
...
- seeds the chosen sub-events from `p_sub_events` — including per-sub-event `event_date`, `start_time`, `end_time`, `venue` (captured in the create wizard Step 3 "Set date & time" / "Set venue" modals; added 2026-06-23). `nullif('')`-coerced so empty values land as NULL;
...
| `public.notifications` | the recipient (`user_id = auth.uid()`) | recipient may UPDATE own rows (mark read); inserts only via RPCs |
...
| Table | Rationale |
|---|---|
| `event_invitation_cards` | Digital Invitations FE is unpersisted (no live route reads/writes it). Convert under a future `'invitations'` capability when wired. |
| `event_media_tags`, `event_media_tag_links` | Schema exists (D37) but no live route touches them. Convert under `'media'` when media-tagging is routed. |
| `notifications` (has `event_id`) | Per-user inbox rows gated by `user_id`, not event-team policies; not an event-child collaborator surface. |
| `event_website_password_sessions`, `guest_lookup_attempts`, `guest_tokens` | Public/guest-facing or rate-limit ledgers — not host collaborator tables; RLS already denies/special-cases client roles. |
| Views (`event_*_summary`, `event_album_counts`, etc.) | `security_invoker` views inherit underlying table policies; no separate conversion. |
