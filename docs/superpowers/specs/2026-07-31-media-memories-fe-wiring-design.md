...
**Photos:**
1. Client picks file(s) → for each, `optimizeImage()` (`lib/storage/imageOptimize.ts`, already built, currently only used by the dev test page) produces `{master, thumb}` WebP blobs client-side (HEIC handled via `heic2any`, keep-smaller guard applies). **HEIC edge case (closes remaining sub-gap from council confirm-pass, Frontend Engineer):** Safari decodes HEIC natively and doesn't need `heic2any` at all — branch on `navigator.userAgent`/feature-detect to skip the conversion step there, avoiding wasted wasm-load cost on the one browser that doesn't need it. On any browser, if `heic2any` throws (some HEIC variants — burst-mode, depth-map, certain iOS HDR/Live Photo captures — are known to reject), that file's pipeline entry goes straight to the same inline error/retry state a failed upload gets, with a message naming the format issue rather than a generic failure.
...
**Council reviewed:** 2026-07-31 by Tech Lead, Backend Engineer, Security Expert, Frontend Engineer, Test Engineer (critique + debate + arbiter). **Original verdict:** 🔴 RE-PLAN — 3 criticals (event-ownership authorization not mandated; thumb/poster key never verified; async mutation UX/rollback contract undefined), ~13 important findings. This revision closes all 3 criticals and the important findings that were addressable without new infrastructure or a product decision (see §10 for what's explicitly deferred and why).

**Confirm-fixes pass (2026-07-31, lighter — 3 agents, single round):** Security Expert, Backend Engineer, Frontend Engineer each re-checked only their own original findings against the revision. **Verdict: 🟢 PROCEED** — all criticals and importants confirmed closed or adequately deferred with stated reasoning; zero regressions introduced by the fixes themselves. One minor open item (Frontend Engineer): HEIC decode-failure UX and a Safari-native-decode skip branch weren't in the first revision — patched into §4 directly. Backend Engineer flagged one thing to watch, not blocking: once real storage-quota enforcement lands (currently out of scope, §10 #10), confirm it doesn't double-count R2 objects whose DB row was deleted under the "DB-first" bulk-delete ordering (§7).

Ready for implementation planning — next step is `superpowers:writing-plans`.
